...
--Fly
addElementToTab("MOVEMENT", "toggle", "Fly", false, function(state)
    -- state = true (включено) или false (выключено)
    if state then
        local Players = game:GetService("Players")
...
local player = Players.LocalPlayer
local flying = true -- Начальное состояние
local flySpeed = 100
local forceDisabled = false -- Флаг принудительного отключения

-- Полная очистка эффектов полёта
...
local function setupFly(character)
    ...
    -- Если полет был принудительно отключен, не включаем его
    if forceDisabled then
        restorePhysics(character)
        return
    end
    ...
    if flying and not forceDisabled then
        -- Настройка режима полёта
        for _, part in ipairs(character:GetDescendants()) do
            if part:IsA("BasePart") then
        ...
        local function flyUpdate()
            if not flying or forceDisabled or not rootPart or not rootPart.Parent then return end
            ...
            if UserInput:IsKeyDown(Enum.KeyCode.W) then direction += forward end
            if UserInput:IsKeyDown(Enum.KeyCode.S) then direction -= forward end
            if UserInput:IsKeyDown(Enum.KeyCode.D) then direction += right end
            if UserInput:IsKeyDown(Enum.KeyCode.A) then direction -= right end
            if UserInput:IsKeyDown(Enum.KeyCode.Space) then direction += up end
            if UserInput:IsKeyDown(Enum.KeyCode.LeftShift) then direction -= up end
        ...
        _G.FlyConnection = RunService.Heartbeat:Connect(flyUpdate)
...
player.CharacterAdded:Connect(function(newChar)
    -- При появлении нового персонажа проверяем, нужно ли включать полет
    if not forceDisabled then
        setupFly(newChar)
    else
        restorePhysics(newChar)
    end
end)
...
-- 1. Отключаем глобальные переменные полета
_G.FLY_ENABLED = false -- Добавляем флаг для контроля состояния

-- 2. Полная очистка эффектов полёта
...
-- 3. Восстановление физики персонажа
local function restorePhysics(character)
    if not character then return end
    
    -- Сначала очищаем все эффекты полета
    cleanUpFlight()
    
    -- Восстанавливаем физику всех частей
    for _, part in ipairs(character:GetDescendants()) do
        if part:IsA("BasePart") then
            part.CanCollide = true
            part.CustomPhysicalProperties = nil
            ...
            -- Удаляем все физические эффекты
            for _, force in ipairs(part:GetChildren()) do
                if force:IsA("BodyVelocity") or force:IsA("BodyGyro") or force:IsA("BodyForce") then
                    force:Destroy()
                end
            end
        end
...
-- 4. Функция для обработки нового персонажа
local function handleCharacter(character)
    restorePhysics(character)
    
    -- Дополнительная защита: отключаем возможные автоматические включения
...
-- 5. Применяем к текущему персонажу
if player.Character then
    handleCharacter(player.Character)
end

-- 6. Обрабатываем респавны
player.CharacterAdded:Connect(function(newChar)
    handleCharacter(newChar)
    
    -- Дополнительная защита от автоматического включения
...
-- 7. Очищаем все при закрытии скрипта
